...
export default function CyclingMapPage() {
  ...
  // Fetch clases de Cycling (room_id=1) de esta semana
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        ...
        // Filtramos solo las de Cycling
        const cycling = (res.data || []).filter((c) => c.room_id === 1);
        setClasses(cycling);
      } catch {
  ...
  return (
    <div style={{ padding: isMobile ? "24px 16px 100px" : "40px 48px" }}>
      ...
      {loading ? (
        ...
        >
          No hay clases de Cycling programadas esta semana
        </div>
      ) : (
        <div
          style={{
      ...
      {selectedClass && (
        <>
          ...
          {/* Posición seleccionada + botón bloquear */}
          {selectedPosition && (
            <div
              style={{
                marginTop: 24,
